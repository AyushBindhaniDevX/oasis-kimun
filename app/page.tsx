...
import { ArrowRight, ShieldCheck, Globe, Users, Award, BookOpen, Target, Play, Image as ImageIcon } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 selection:bg-blue-100 selection:text-blue-700">
      ...
      <main className="max-w-5xl mx-auto px-6 space-y-32 pb-32">
        ...
        {/* NEW Gallery & Highlights Section */}
        <section className="space-y-12">
          <div className="text-center space-y-4">
            <h2 className="text-xs font-bold text-blue-600 uppercase tracking-[0.3em]">Highlights</h2>
            <h3 className="text-3xl font-bold text-slate-900">Last Year at KIMUN</h3>
          </div>
          
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Main Video Slot */}
            <div className="lg:col-span-2 relative aspect-video rounded-[2rem] overflow-hidden border border-slate-100 bg-slate-50 group">
              {/* Replace URL below with your YouTube Embed URL */}
              <iframe 
                className="w-full h-full"
                src="https://www.youtube.com/embed/DTOmNjbOR4o" 
                title="KIMUN Last Year Highlights"
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
                allowFullScreen
              ></iframe>
              <div className="absolute top-6 left-6 bg-white/90 backdrop-blur px-4 py-2 rounded-full flex items-center gap-2 shadow-sm pointer-events-none">
                <Play className="w-3 h-3 text-blue-600 fill-blue-600" />
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-900">Aftermovie</span>
              </div>
            </div>

            {/* Photo Grid Column */}
            <div className="grid grid-rows-2 gap-6">
              <div className="relative rounded-[2rem] overflow-hidden border border-slate-100 bg-slate-50 group">
                <img 
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
                   <span className="text-white text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                     <ImageIcon className="w-3 h-3" /> Committee Sessions
                   </span>
                </div>
              </div>
              <div className="relative rounded-[2rem] overflow-hidden border border-slate-100 bg-slate-50 group">
                <img 
                  src="https://kimun497636615.wordpress.com/wp-content/uploads/2026/03/img_1090.jpg" 
                  alt="KIMUN Delegates" 
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
                   <span className="text-white text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                     <ImageIcon className="w-3 h-3" /> Networking
                   </span>
                </div>
              </div>
            </div>
